-- 20260217110000_update_notification_targets.sql
-- Updates target_path in notification triggers to match v1.0.7 redirection policies.

-- 1. Update Family Notification Targets
    ...
    IF target_client_id IS NOT NULL THEN
        ...
        VALUES (
            ...
            jsonb_build_object(
                ...
                'target_path', '/dashboard' -- Changed from conditional to fixed Dashbaord
            )
        );
    END IF;
    RETURN NEW;
...
-- 2. Update Caregiver Notification Targets
    ...
    IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status IN ('approved', 'rejected') THEN
        ...
        VALUES (
            ...
            jsonb_build_object(
                'appointment_id', NEW.appointment_id,
                'status', NEW.status,
                'target_path', '/caregiver/jobs' -- Both approved/denied go to Jobs board per policy
            )
        );
    END IF;
    RETURN NEW;
...
-- 3. Update Care Log Targets (Activity/Wellness -> BC PULSO)
...
BEGIN
    ...
    VALUES (
        ...
        jsonb_build_object(
            'appointment_id', NEW.appointment_id,
            'log_id', NEW.id,
            'target_path', '/dashboard/pulso' -- Standard target for all Pulse activities
        )
    );
    RETURN NEW;
END;
...
-- 4. Notify Caregiver on Direct Request (Family -> Cuidador)
-- Ensure direct requests lead to Caregiver Dashboard
CREATE OR REPLACE FUNCTION notify_caregiver_on_direct_request()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO notifications (user_id, type, title, message, metadata)
    VALUES (
        ...
        'Has recibido una solicitud de trabajo. ¡Revisa los detalles!',
        jsonb_build_object(
            'appointment_id', NEW.appointment_id,
            'target_path', '/caregiver' -- Caregiver Dashboard
        )
    );
    RETURN NEW;
END;
