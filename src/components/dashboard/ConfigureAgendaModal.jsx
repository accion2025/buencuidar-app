...
import { X, ShieldCheck, ChevronDown, ChevronRight, Check, Save, Loader2, Brain, Activity, ClipboardList, Heart, Stethoscope, UserCog, Home } from 'lucide-react';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const agendaArray = Object.entries(selectedActivities).map(([activity, time]) => {
                ...
                const category = CARE_AGENDA_CATEGORIES.find(cat => {
                    const catActivities = cat.activities || (cat.sections ? cat.sections.flatMap(s => s.activities) : []);
                    return catActivities.includes(activity);
                });

                return {
                    activity,
                    time,
                    program_name: category ? category.name : null
                };
            });
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 animate-fade-in shadow-2xl">
            <div className="bg-white rounded-none sm:rounded-[16px] shadow-2xl w-full max-w-4xl h-full sm:h-auto max-h-full sm:max-h-[90vh] flex flex-col border border-white/20 overflow-hidden relative">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-10 space-y-5 sm:space-y-8 custom-scrollbar pb-32 sm:pb-10 pb-safe">
                    {!can('manualAgendaUpdate') ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center space-y-8 animate-fade-in">
                            <div className="bg-[var(--secondary-color)]/10 p-8 rounded-[32px] text-[var(--secondary-color)] shadow-inner">
                                <ShieldCheck size={64} strokeWidth={1.5} />
                            </div>
                            <div className="max-w-md space-y-4">
                                <h4 className="text-3xl font-brand font-bold text-[var(--primary-color)]">Agenda de Cuidado Profesional</h4>
                                <p className="text-lg text-[var(--text-light)] font-secondary leading-relaxed">
                                    La configuración manual de tareas es una función avanzada para garantizar la mejor atención. Actualiza a <span className="font-black text-[var(--secondary-color)]">BC PULSO</span> para desbloquear esta herramienta.
                                </p>
                            </div>
                            <button
                                onClick={() => navigate('/dashboard/plans')}
                                className="btn btn-primary px-12 py-5 text-lg shadow-2xl uppercase tracking-widest rounded-[18px]"
                            >
                                Ver Planes Premium
                            </button>
                        </div>
                ...
                <div className="shrink-0 p-6 sm:p-10 border-t border-gray-100 bg-white shadow-inner sm:bg-white/95 sm:backdrop-blur-2xl flex flex-col sm:flex-row justify-between items-center gap-6 z-30 pb-safe">
                    ...
                    <div className="flex gap-4 w-full sm:w-auto">
                        ...
                        <button
                            onClick={handleSave}
                            disabled={saving || !can('manualAgendaUpdate')}
                            className={`flex-[2] sm:flex-none btn btn-primary px-12 sm:px-16 py-5 shadow-2xl uppercase tracking-widest text-sm sm:text-lg rounded-[18px] ${(!can('manualAgendaUpdate')) ? 'opacity-50 cursor-not-allowed grayscale' : 'shadow-green-900/20'}`}
                        >
