...
const AppointmentsListModal = ({ isOpen, onClose, appointments, onEdit, onDelete }) => {
    ...
    const sortedAppointments = [...appointments]
        .filter(app => app.status !== 'cancelled' && app.type !== 'Cuidado+')
                    ...
                    {sortedAppointments.length > 0 ? (
                        sortedAppointments.map((item) => (
                            <div key={item.id} className="bg-white border border-gray-100 rounded-[16px] p-5 hover:shadow-md transition-shadow flex flex-col sm:flex-row gap-4 sm:items-center group">
                                {/* Date Box */}
                                <div className="rounded-[16px] p-3 flex flex-col items-center justify-center min-w-[80px] bg-blue-50 text-blue-600">
                                ...
                                <div className="flex sm:flex-col items-center sm:items-end gap-2 text-xs">
                                    ...
                                    <div className="flex gap-2 mt-1">
                                        ...
                                        {onEdit && (
                                            <button
                                                onClick={() => onEdit(item)}
                                                className="px-3 py-1.5 bg-blue-50 text-blue-600 font-black rounded-[16px] hover:bg-blue-100 transition-all flex items-center gap-1.5 border border-blue-100 shadow-sm"
                                                title="Editar"
                                            >
                                                <Pencil size={14} />
                                                EDITAR
